...
class JAXAgent(embodied.Agent):
    def __init__(self, agent_cls, obs_space, act_space, step, config):
        ...
        self.batch_size = config.batch_size // 2 if self.agent.wm.demo_bc else config.batch_size
    ...
    def train(self, data, state=None, reference_data=None):
        ...
        if state is None:
            if self.agent.wm.demo_bc:
                state, self.varibs = self._init_train(self.varibs, rng, np.repeat(data["is_first"], 2, axis=0))
            else:
                state, self.varibs = self._init_train(self.varibs, rng, data["is_first"])
        if reference_data is not None:
            (outs, state, mets), self.varibs = self._train(self.varibs, rng, data, state, reference_data)
        else:
    ...
    def _init_varibs(self, obs_space, act_space):
        ...
        if self.agent.wm.demo_bc:
            state, varibs = self._init_train(varibs, rng, np.repeat(data["is_first"], 2, axis=0))
            ...
            varibs = self._train(varibs, rng, data, state, reference_data, init_only=True)
        elif self.agent.wm.amp:
