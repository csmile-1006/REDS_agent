...
from reds_rl.clip import model as clip
...
class Discriminator(nj.Module):
    def __init__(
        ...
        **kw,
        ...
        shapes = {k: v for k, v in shapes.items() if (not re.match(excluded, k) and not k.startswith("log_"))}
        ...
        if cnn == "resnet":
            ...
            self._cnn = CLIPEncoder()
        else:
            raise NotImplementedError(cnn)
        logit_kw = {**kw, "symlog_inputs": symlog_inputs, "name": "logit_mlp"}
        self._logit_mlp = MLP(layers=mlp_layers, units=mlp_units, dims="tensor", shape=None, **logit_kw)
...
class MultiDecoder(nj.Module):
    ...
    def __call__(self, inputs, drop_loss_indices=None):
        ...
        if self.cnn_shapes:
            ...
            dists.update(
                {key: self._make_image_dist(key, mean) for (key, shape), mean in zip(self.cnn_shapes.items(), means)}
            )
        if self.mlp_shapes:
...
class CLIPEncoder(object):
    def __init__(self):
        ...
        self.clip_model = clip.MODELS[model_name]()
        self.clip_vars = clip.load_model_vars(model_name)

    def __call__(self, x):
        B = x.shape[0]
        images = jnp.reshape(x, (-1,) + x.shape[-3:])
        ...
        image_features = self.clip_model.apply(
            self.clip_vars, images, method=self.clip_model.encode_image, normalize=False
        )
        image_features = jnp.reshape(image_features, (B, -1))
        return jax.lax.stop_gradient(image_features)
