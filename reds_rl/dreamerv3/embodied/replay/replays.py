...
class UniformRelabel(generic.Generic):
    ...
    def add_chunk(self, step, worker=0, load=False):
        ...
        next_step_processed = self.reward_model.is_step_processed(stream[self.reward_model.seq_len_steps - 1])
        is_last_idxs = [i for i, step in enumerate(stream) if step["is_last"] or step["is_terminal"]]
        seq_has_last_step = (len(is_last_idxs) == 1) and (is_last_idxs[-1] == len(stream) - 1)
        ...
        if len(is_last_idxs) > 0 and not seq_has_last_step:
            ...
            for i in range(len(stream)):
                if not self.reward_model.is_step_processed(stream[i]):
                    split_idx = i
                    break
            seq = self.reward_model(tuple(stream))[split_idx - self.reward_model.seq_len_steps + 1 :]
        else:
            return
