...
def collect_demo(
    ...
    for idx in (pbar := tqdm(range(num_demos))):
        success = False
        while not success:
            try:
                demo, demo_randomize = env._task.get_demos(
                    1, live_demos=True, randomize=randomize, collect_suboptimal=True
                )
                demo = demo[0]
                demo_randomize = demo_randomize[0]
                success = True
                pbar.set_postfix({"demo_length": len(demo)})
            except:
                pass
        if randomize:
            trns = extract_from_paired_demo(
                demo,
                demo_randomize,
                reward_type,
                camera_keys,
            ...
            trns = extract_from_demo(
                demo,
                reward_type,
                camera_keys,
                use_rotation,
                additional_camera=additional_camera,
                use_tailing=use_tailing,
            )
        transitions.extend(trns)
        if save_dir:
            # Convert list of dicts to dict of lists
            trns_dict = {key: [] for key in trns[0].keys()}
            for item in trns:
                for key, value in item.items():
                    trns_dict[key].append(value)
            
            success = np.sum(trns_dict["success"][-1]) > 0
            # Save the dict of lists
            np.save(save_dir + f"/{'success' if success else 'failure'}_demo_{idx}.npy", trns_dict)
    ...
    for obs in transitions:
        if obs["is_first"]:
            ...
            if use_rotation:
                ...
                norm_theta_delta = 2 * ((theta_delta - rot_low) / (rot_high - rot_low)) - 1
            ...
            obs["reset"] = obs["is_last"]

        demo_replay.add(obs)

    if len(transitions) == 0:
        print("No transitions found. Setting min/max to None.")
...
def extract_from_demo(
    ...
    for k, obs in enumerate(demo):
        if k == 0:
            ...
            if use_rotation:
                theta_deltas, prev_theta, use_minus = get_theta_delta(prev_theta, obs, use_minus)
        ...
        if shaped_rewards:
            ...
            skill = obs.task_low_dim_state[1]
        else:
