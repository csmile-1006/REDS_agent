...
def _encode_gif(frames, fps):
    ...
    process = (
        ffmpeg.input("pipe:", format="rawvideo", s="{}x{}".format(w, h), r=fps, pix_fmt=pxfmt)
        .output(
            "pipe:", format="gif", r=fps, filter_complex="[0:v]split[x][z];[z]palettegen[y];[x]fifo[x];[x][y]paletteuse"
        )
