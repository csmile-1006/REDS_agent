...
class ExpandScalars(base.Wrapper):
    def __init__(self, env):
        ...
        for key, space in self.env.obs_space.items():
            if space.shape == () and key != "reward" and key != "density" and key != "success" and not space.discrete:
...
class Density(base.Wrapper):
    ...
    def obs_space(self):
        ...
        spaces["density"] = spacelib.Space(np.float32)
        return spaces


class Rank(base.Wrapper):
    def __init__(self, env):
        super().__init__(env)
    ...
    def obs_space(self):
        spaces = self.env.obs_space
        spaces["rank"] = spacelib.Space(np.float32)
        return spaces


class VQCodes(base.Wrapper):
    def __init__(self, env, reward_model, vqcodes):
        super().__init__(env)
        self.reward_model = reward_model
        self.vqcodes = vqcodes
    ...
    def obs_space(self):
        ...
        for key in spaces.keys():
            if re.match(self.vqcodes, key):
                new_spaces[f"codes_{key}"] = spacelib.Space(
                    self.reward_model.codes_dtype,
                    self.reward_model.codes_size,
                    low=0,
                    high=self.reward_model.ae.ae.n_embed,
                )
                new_spaces[f"embed_{key}"] = spacelib.Space(self.reward_model.embed_dtype, self.reward_model.embed_size)
        spaces.update(new_spaces)
        return spaces

    def step(self, action):
        obs = self.env.step(action)
        images_to_encode, images_to_encode_keys = [], []
        for key, val in obs.items():
            if re.match(self.vqcodes, key):
                images_to_encode_keys.append(key)
                images_to_encode.append(val)

        images_to_encode = np.stack(images_to_encode, axis=0)
        images_to_encode = self.reward_model.process_images(images_to_encode)
        encodings, embeddings = self.reward_model.ae.encode(images_to_encode, include_embeddings=True)
        new_obs = {}
        for i, key in enumerate(images_to_encode_keys):
            new_obs[f"codes_{key}"] = np.array(encodings[i])
            new_obs[f"embed_{key}"] = np.array(embeddings[i])
        obs.update(new_obs)
        return obs


class CLIP(base.Wrapper):
    def __init__(self, env, lang_key, encode_text_fn, tokenize_with_mask_fn):
        super().__init__(env)
        self.lang_key = lang_key
        self.cache = {}
        self.encode_text = encode_text_fn
        self.tokenize_with_mask = tokenize_with_mask_fn

    @functools.cached_property
    def obs_space(self):
        spaces = self.env.obs_space
        spaces[f"text_mask_{self.lang_key}"] = spacelib.Space(bool, shape=(16,))
        spaces[f"text_tokens_{self.lang_key}"] = spacelib.Space(np.int32, shape=(16))
        spaces[f"text_{self.lang_key}"] = spacelib.Space(np.float32, shape=(16, 768))
        spaces[f"text_flat_{self.lang_key}"] = spacelib.Space(np.float32, shape=(768))
        return spaces
    ...
    def step(self, action):
        ...
        del_keys = []
        for key, val in obs.items():
            if re.match(self.lang_key, key):
                if val not in self.cache:
                    # Cache clip embeddings.
                    text_tokens, text_mask = self.tokenize_with_mask(val, context_length=16)
                    text, text_global = self.encode_text(text_tokens)
                    self.cache[val] = {
                        f"text_mask_{key}": text_mask[0],
                        f"text_tokens_{key}": text_tokens[0],
                        f"text_{key}": text[0],
                        f"text_flat_{key}": text_global[0],
                    }
                new_obs.update(self.cache[val])
                del_keys.append(key)
        for key in del_keys:
            del obs[key]
        obs.update(new_obs)
