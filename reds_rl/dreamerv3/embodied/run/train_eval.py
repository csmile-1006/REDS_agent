...
def train_eval(
    agent, train_env, eval_env, train_env_fn, eval_env_fn, train_replay, eval_replay, demo_replay, logger, args
):
    ...
    def per_episode(ep, mode):
        ...
        if "success" in ep:
            ...
            logger.add(
                {f"{mode}_success": float(success)}, prefix=("episode" if mode == "train" else f"{mode}_episode")
            )
            print(f"{mode.title()} episode has {float(success)} success, {length} steps and return {score:.1f}.")
        else:
            print(f"{mode.title()} episode has {length} steps and return {score:.1f}.")
        stats = {}
        for key in args.log_keys_video:
            if key in ep and mode == "eval":
    ...
    if demo_replay is not None:
        dataset_demo = agent.dataset(demo_replay.dataset)
    ...
    def train_step(tran, worker):
        for _ in range(should_train(step)):
            ...
            if demo_replay is not None:
                with timer.scope("reference_dataset"):
                    demo_batch[0] = next(dataset_demo)
            outs, state[0], mets = agent.train(batch[0], state[0], demo_batch[0])
        ...
        if should_log(step):
            logger.add(metrics.result())
            logger.add(agent.report(batch[0], demo_batch[0]), prefix="report")
            with timer.scope("dataset_eval"):
                eval_batch = next(dataset_eval)
            logger.add(agent.report(eval_batch, demo_batch[0]), prefix="eval")
