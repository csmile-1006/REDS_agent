...
class Shortcut(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        x = nn.Conv(self.features, (1, 1), strides=(1, 1), use_bias=False, name="0")(x)
...
class Bottleneck(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        conv2 = nn.Conv(self.features, (3, 3), padding=[(1, 1), (1, 1)], use_bias=False, name="conv2")
        bn2 = nn.BatchNorm(use_running_average=True, name="bn2")

        conv3 = nn.Conv(self.features * self.expansion, (1, 1), use_bias=False, name="conv3")
        bn3 = nn.BatchNorm(use_running_average=True, name="bn3")

        out = nn.relu(bn1(conv1(x)))
        out = nn.relu(bn2(conv2(out)))
        out = nn.avg_pool(out, (self.stride, self.stride), (self.stride, self.stride))
        out = bn3(conv3(out))

        downsample = self.stride > 1 or x.shape[-1] != self.features * self.expansion
        if downsample:
            x = Shortcut(features=self.features * self.expansion, stride=self.stride, name="downsample")(x)
...
class AttentionPool(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        positional_embedding = self.param(
            "positional_embedding", jax.nn.initializers.normal(1.0 / x.shape[-1] ** 0.5), (x.shape[1], x.shape[2])
        )
        attn = nn.MultiHeadDotProductAttention(
            self.num_heads, qkv_features=x.shape[-1], use_bias=True, out_features=self.features, name="attn"
        )
...
class ModifiedResNet(nn.Module):
    ...
    def setup(self):
        ...
        self.conv2 = nn.Conv(
            self.features // 2, kernel_size=(3, 3), padding=[(1, 1), (1, 1)], use_bias=False, name="conv2"
        )
        self.bn2 = nn.BatchNorm(use_running_average=True, name="bn2")
        self.conv3 = nn.Conv(self.features, kernel_size=(3, 3), padding=[(1, 1), (1, 1)], use_bias=False, name="conv3")
        ...
        self.layer2 = ResNetStage(self.features * 2, self.num_layers[1], stride=2, name="layer2")
        self.layer3 = ResNetStage(self.features * 4, self.num_layers[2], stride=2, name="layer3")
        self.layer4 = ResNetStage(self.features * 8, self.num_layers[3], stride=2, name="layer4")
        if self.out_features is not None:
            self.attnpool = AttentionPool(self.num_heads, self.out_features, name="attnpool")
...
class ResidualAttentionBlock(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, attn_mask=None) -> jnp.ndarray:
        ...
        x = x + nn.SelfAttention(self.num_heads, name="attn", deterministic=True)(xn, attn_mask)
...
class Transformer(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, attn_mask: Optional[jnp.ndarray] = None) -> jnp.ndarray:
        ...
        for i in range(self.num_layers):
            x = ResidualAttentionBlock(num_heads=self.num_heads, name=_n(f"resblocks.{i}"))(x, attn_mask)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, attn_mask: Optional[jnp.ndarray] = None) -> jnp.ndarray:
        ...
        class_embedding = self.param("class_embedding", jax.nn.initializers.normal(stddev=scale), (self.features,))
        x = jnp.concatenate((jnp.tile(class_embedding[None, None, :], (x.shape[0], 1, 1)), x), axis=1)
        positional_embedding = self.param(
            "positional_embedding", jax.nn.initializers.normal(stddev=scale), (x.shape[1], self.features)
        )
...
class TextEncoder(nn.Module):
    ...
    def __call__(self, text: jnp.ndarray) -> jnp.ndarray:
        positional_embedding = self.param(
            "positional_embedding", jax.nn.initializers.zeros, (text.shape[1], self.features)
        )
