...
DEFAULT_DOWNLOAD_DIR = os.path.expanduser("~/.cache/clip")
...
def download(url: str, root: str = DEFAULT_DOWNLOAD_DIR, expected_sha256: Optional[str] = None):
    ...
    if gfile.exists(download_target):
        if gfile.isdir(download_target):
            ...
            logging.warning(
                "%s exists, but the SHA256 checksum does not match;" "re-downloading the file", download_target
            )
