...
def _tokenize(
    texts: Union[str, Sequence[str]], tokenizer: Any, context_length: int, truncate: bool = False
) -> np.ndarray:
...
def build_tokenizer(
    ...
    tokenizer_fn = functools.partial(_tokenize, tokenizer=tokenizer, context_length=MAX_TEXT_LENGTH, truncate=truncate)
    return tokenizer_fn
