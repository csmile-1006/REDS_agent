...
def load_dataset(config, train, modality="video"):
    ...
    def get_dataset(data_path, data_type, batch_n, initial_shape, mask):
        ...
        dataset = load_fn(config, data_path, train, num_ds_shards, ds_shard_id, mask)

        if modality == "image":

            def to_images(features):
                video = features.pop("video")
                assert len(features) == 0, "Only support video-only dict"
    ...
    def get_data_type(data_path):
        ...
        if len(fns) >= 2 and osp.exists(osp.join(data_path, "val")):  # 'train' and 'test' directories
            fns = _glob_files(osp.join(data_path, "val", "*"))
            if "mp4" in fns[0]:
                return "mp4", None
            elif "pkl" in fns[0]:
                return "pkl", None
    ...
    if data_type in ["mp4", "npz", "pkl"]:
        dataset = get_dataset(config.data_path, data_type, batch_size, initial_shape=(N, batch_size // N), mask=None)
...
def load_npz(config, data_path, train, n_shards, shard_id, mask):
    ...
    folder = osp.join(data_path, split, "*_success.npz")
    ...
    def read(path):
        ...
        video = np.asarray([elem[config.image_key] for elem in ep["observations"]])
    ...
    def process(video):
        if hasattr(config, "seq_len"):
            ...
            video = tf.reshape(video, tf.concat(([tf.shape(video)[0] // req_len, req_len], tf.shape(video)[1:]), 0))
    ...
    dataset = dataset.map(lambda item: tf.numpy_function(read, [item], [tf.uint8]), num_parallel_calls=tf.data.AUTOTUNE)
    dataset = dataset.flat_map(process)
    dataset = dataset.map(lambda video: dict(video=video))
    return dataset


def load_pkl(config, data_path, train, n_shards, shard_id, mask):
    split = "train" if train else "val"
    folder = osp.join(data_path, split, "*_success.pkl")
    fns = _glob_files(folder)
    random.Random(1234).shuffle(fns)
    assert len(fns) > 0, f"Could not find any files for {folder}"
    fns = np.array_split(fns, n_shards)[shard_id].tolist()
    ...
    def read(path):
        ...
        for obs in ep["observations"]:
            image = obs[config.image_key]
            if image.shape[0] == 3:
                image = image.transpose(1, 2, 0)
            if image.shape[0] != config.image_size:
                image = np.array(
                    Image.fromarray(image).resize((config.image_size, config.image_size), Image.Resampling.NEAREST)
                )
            video.append(image)
