...
class VideoGPTRewardModel:
    ...
    def __init__(
        ...
        if self.class_cond and self.class_map is not None:
            ...
            assert self.task in self.class_map, f"{self.task} not found in class map."
            self.task_id = int(self.class_map[self.task])
            print(f"Loaded conditioning information for task {self.task}")
        elif self.class_cond:
            raise ValueError(
                f"No class_map for class_conditional model. " f"VideoGPT loaded class_map? {self.class_map is not None}"
            )
        else:
            self.task_id = None
        ...
        print(
            ...
            f"\n\tn_skip? {self.n_skip}"
            f"\n\tbatch_size? {self.encoding_minibatch_size} {self.minibatch_size}"
        )
    ...
    def rollout_video(
        self, init_frames, video_length, seed=0, open_loop_ctx=4, inputs_are_codes=False, decode=True, pbar=False
    ):
        if inputs_are_codes:
        ...
        batch = dict(encodings=encodings, label=self.expand_scalar(self.task_id or 0, encodings.shape[0], jnp.int32))
        if rollout_length > init_frames.shape[1]:
            ...
            encodings = self.sampler(
                self.variables, batch=batch, log_tqdm=False, seed=seed, open_loop_ctx=init_frames.shape[1], decode=False
            )
            batch["encodings"] = encodings
        ...
        for _ in vid_range:
            ...
            encodings = self.sampler(
                self.variables, batch=batch, log_tqdm=False, seed=seed, open_loop_ctx=open_loop_ctx, decode=False
            )
    ...
    def _compute_likelihood(self, variables, embeddings, encodings, label):
        ...
        if self.n_skip > 1:
            ...
            print(
                f"\tAfter applying frame skip: Embeddings shape: {embeddings.shape}, Encodings shape: {encodings.shape}"
            )
        likelihoods = self.model.apply(
            variables, embeddings, encodings, label=label, reduce_sum=self.nll_reduce_sum, method=self.model.log_prob
        )
    ...
    def _compute_likelihood_for_initial_elements(self, variables, embeddings, encodings, label):
        print(
            f"Tracing init frame likelihood: Embeddings shape: {embeddings.shape}, Encodings shape: {encodings.shape}"
        )
        if self.n_skip > 1:
            ...
            print(
                f"\tAfter applying frame skip: Embeddings shape: {first_embeddings.shape}, Encodings shape: {first_encodings.shape}"
            )
        else:
        ...
        likelihoods = self.model.apply(
            ...
            first_encodings,
            label=label,
            reduce_sum=self.nll_reduce_sum,
    ...
    def compute_reward(self, seq: List[Dict[str, Any]], **kwargs):
        ...
        for i in range(0, len(image_batch), self.minibatch_size):
            _image_batch = image_batch[i : (i + self.minibatch_size)]
            _encodings = self.ae.encode(jnp.expand_dims(_image_batch, axis=0))
            _embeddings = self.ae.lookup(_encodings)
            _encodings, _embeddings = _encodings[0], _embeddings[0]
            encodings.append(_encodings)
            embeddings.append(_embeddings)

        encodings = jnp.concatenate(encodings, axis=0)
        ...
        for i in range(0, len(idxs), self.minibatch_size):
            ...
            rewards.append(sg(self._compute_likelihood(self.variables, mb_embeddings, mb_encodings, mb_label)))
        rewards = jnp.concatenate(rewards, axis=0)
        if len(rewards.shape) <= 1:
            rewards = self._reward_scaler(rewards)
        if self.use_task_reward:
            rewards = rewards + jnp.array([seq[i]["reward"] for i in idxs])
        ...
        if seq[0]["is_first"]:
            ...
            first_rewards = sg(
                self._compute_likelihood_for_initial_elements(
            ...
            if self.use_task_reward:
                first_rewards = first_rewards + jnp.array([seq[i]["reward"] for i in range(self.seq_len_steps - 1)])
