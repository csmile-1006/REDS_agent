...
class VQPerceptualWithDiscriminator:
    ...
    @property
    def metrics(self):
        return ["loss_G", "loss_D"] + self.vqgan.metrics + self.disc.metrics + ["d_weight"]

    def loss_recon(self, vqgan_params, lpips_variables, batch, rng):
        rng, old_rng = jax.random.split(rng)

        out = self.vqgan.apply({"params": vqgan_params}, batch["image"], deterministic=False, rngs={"dropout": old_rng})
        ...
        if self.config.perceptual_weight > 0:
            ...
            p_loss = jnp.array([0.0], dtype=jnp.float32)
        rec_loss = rec_loss + self.config.perceptual_weight * p_loss
        out["ae_loss"] = jnp.mean(rec_loss)
        loss = out["ae_loss"] + self.config.codebook_weight * out["vq_loss"]
        return loss, (out, rng)

    def loss_G(self, vqgan_params, disc_params, disc_model_state, lpips_variables, batch, rng):
        _, (vqgan_out, rng) = self.loss_recon(vqgan_params, lpips_variables, batch, rng)

        g_out = self.disc.loss_G(disc_params, disc_model_state, vqgan_out["recon"])

        d_weight = jnp.array(self.config.disc_weight, dtype=jnp.float32)

        loss = vqgan_out["ae_loss"] + d_weight * g_out["g_loss"] + self.config.codebook_weight * vqgan_out["vq_loss"]
    ...
    def loss_D(self, disc_params, disc_model_state, vqgan_params, batch, rng):
        ...
        d_out, disc_model_state = self.disc.loss_D(disc_params, disc_model_state, batch["image"], out["recon"])
