...
class TrainStateVQ(flax.struct.PyTreeNode):
    ...
    def apply_vqgan_gradients(self, *, vqgan_grads, **kwargs):
        updates, new_vqgan_opt_state = self.tx.update(vqgan_grads, self.vqgan_opt_state, self.vqgan_params)
    ...
    def apply_disc_gradients(self, *, disc_grads, **kwargs):
        updates, new_disc_opt_state = self.tx.update(disc_grads, self.disc_opt_state, self.disc_params)
    ...
    @classmethod
    def create(cls, *, vqgan_params, disc_params, disc_model_state, lpips_variables, tx):
...
def get_optimizer(config):
    learning_rate_fn = optax.join_schedules(
        [
            optax.linear_schedule(init_value=0.0, end_value=config.lr, transition_steps=config.warmup_steps),
...
def init_model_state_videogpt(rng, model, batch, config):
    variables = model.init(rngs={"params": rng, "dropout": rng}, **batch, training=True, method=model.loss).unfreeze()
...
def init_model_state_vqgan(rng, model, batch, config):
    variables = model.vqgan.init(
        rngs={"params": rng, "dropout": rng}, image=batch["image"], deterministic=False
    ).unfreeze()
    vqgan_params = variables.pop("params")
    print_model_size(vqgan_params, name="vqgan")

    variables = model.disc.init(rngs={"params": rng}, image=batch["image"], deterministic=False).unfreeze()
...
def save_image_grid(images, fname=None, nrow=None):
    ...
    ncol = math.ceil(b / nrow)
    padding = 1
    image_grid = np.zeros(((padding + h) * ncol + padding, (padding + w) * nrow + padding, c), dtype="uint8")
    for i in range(b):
        r = i // nrow
        c = i % nrow
