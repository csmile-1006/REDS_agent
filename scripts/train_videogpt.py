...
from reds_rl.videogpt.models import AE, VideoGPT
from reds_rl.videogpt.sampler import VideoGPTSampler
from reds_rl.videogpt.data import load_dataset
from reds_rl.videogpt.train_utils import (
    init_model_state_videogpt,
    get_first_device,
    ProgressMeter,
    save_video_grid,
    add_border,
...
def main():
    ...
    while iteration <= config.total_steps:
        iteration, state, rngs = train(iteration, ae, model, state, train_loader, schedule_fn, rngs)
...
def train_step(batch, state, rng):
    def loss_fn(params, batch, rng):
        ...
        out = state.apply_fn(variables, **batch, training=True, rngs={"dropout": rng_dropout}, method=model.loss)
    ...
    if config.ema:
        ...
        ema_params = jax.tree_util.tree_map(
            lambda a, b: decay * a + (1.0 - decay) * b, state.ema_params, new_state.params
        )
...
def train(iteration, ae, model, state, train_loader, schedule_fn, rngs):
    ...
    while True:
        ...
        if is_master_process:
            ...
            wandb.log(jax.device_get({**{f"train/{metric}": val for metric, val in metrics.items()}}), step=iteration)
        ...
        if (
            iteration % config.viz_interval == 0
            or iteration % config.test_interval == 0
            or iteration % config.save_interval == 0
...
def val_step(batch, state, rng):
    def loss_fn(params, batch, rng):
        rng_dropout, new_rng = jax.random.split(rng)
        variables = {"params": params}
        out = state.apply_fn(variables, **batch, training=False, rngs={"dropout": rng_dropout}, method=model.loss)
        return out, new_rng
...
def validate(iteration, ae, model, state, test_loader, rngs):
    progress = ProgressMeter(50, ["time", "data"] + model.metrics, prefix="\tTest:")
    ...
    metrics = {metric: progress.meters[metric].avg for metric in model.metrics}

    if is_master_process:
        wandb.log({**{f"val/{metric}": val for metric, val in metrics.items()}}, step=iteration)
...
def visualize(sampler, ae, iteration, state, test_loader):
    ...
    add_border(samples[:, : config.open_loop_ctx], (0.0, 1.0, 0.0))
    add_border(samples[:, config.open_loop_ctx :], (1.0, 0.0, 0.0))
