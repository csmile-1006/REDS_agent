...
def main(argv=None):
    ...
    try:
        ...
        actions_min_max = embodied.rlbench_collect_demo(
            ...
            use_tailing=False,
            save_dir=config.reference_dir,
        )
        with open(config.reference_dir + "/actions_min_max.pkl", "wb") as f:
...
def make_replay(config, directory=None, is_eval=False, rate_limit=False, reward_model=None, **kwargs):
    ...
    replay = embodied.replay.NaiveChunks(length, size, directory, chunks=1024)
    return replay
