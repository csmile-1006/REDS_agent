...
from reds_rl.videogpt.loss_vqgan import VQPerceptualWithDiscriminator
from reds_rl.videogpt.data import load_dataset
from reds_rl.videogpt.train_utils import init_model_state_vqgan, ProgressMeter, save_image_grid, get_first_device
...
def main():
    ...
    if is_master_process:
        wandb.init(project="reds_rl", config=config, id=config.run_id, resume="allow", mode="online")
    ...
    only_ae_steps = config.only_ae_steps if hasattr(config, "only_ae_steps") else 0
    while iteration <= config.total_steps:
        only_ae = iteration <= only_ae_steps
        iteration, state, rngs = train(iteration, state, train_loader, lr_fn, only_ae, rngs)
...
def train_step(batch, state, rng):
    # Generator update
    aux_G, grads_G = jax.value_and_grad(model.loss_G, has_aux=True)(
        state.vqgan_params, state.disc_params, state.disc_model_state, state.lpips_variables, batch, rng
    )
    aux_G, rng = aux_G[1]
    grads_G = jax.lax.pmean(grads_G, "device")
    state = state.apply_vqgan_gradients(vqgan_grads=grads_G)

    # Discriminator update
    aux_D, grads_D = jax.value_and_grad(model.loss_D, has_aux=True)(
        state.disc_params, state.disc_model_state, state.vqgan_params, batch, rng
    )
    aux_D, disc_model_state, rng = aux_D[1]
    grads_D = jax.lax.pmean(grads_D, "device")
    state = state.apply_disc_gradients(disc_grads=grads_D, disc_model_state=disc_model_state)

    aux = {**aux_G, **aux_D}
    return state, aux, rng


def train_ae_step(batch, state, rng):
    # Generator update
    aux_G, grads_G = jax.value_and_grad(model.loss_recon, has_aux=True)(
        state.vqgan_params, state.lpips_variables, batch, rng
    )
    aux_G, rng = aux_G[1]
    grads_G = jax.lax.pmean(grads_G, "device")
    state = state.apply_vqgan_gradients(vqgan_grads=grads_G)
...
def train(iteration, state, train_loader, lr_fn, only_ae, rngs):
    progress = ProgressMeter(config.total_steps, ["time", "data"] + model.metrics)
    ...
    while True:
        ...
        if only_ae:
            state, metrics, rngs = jax.pmap(train_ae_step, "device")(batch, state, rngs)
            metrics = jax.device_get({k: metrics[k].mean() for k in model.vqgan.metrics})
        else:
        ...
        if (
            ...
            or iteration % config.viz_interval == 0
            or iteration >= config.total_steps
        ):
